...
pub trait Menu {
    ...
    fn change_menu(&mut self, world: &mut World, event: &Event) {
        ...
        let num_items = self.get_menu_ids().len();
        ...
        let menu_entities: Vec<_> = world.exec(|ui_finder: UiFinder| self.get_menu_ids().iter().filter_map(|id| ui_finder.find(id)).collect());
        if menu_entities.len() == num_items {
            ...
            self.write_cursor_alpha_color(ui_texts.get_mut(menu_entities[new_selection as usize]), 1.0);
        }
    }
